				...
				<scTable ref="table" :apiObj="listApi" row-key="id" :params="listApiParams" @selection-change="selectionChange" stripe :paginationLayout="'prev, pager, next'">
					...
					<el-table-column label="是否有效" prop="status" width="100">
						<template #default="scope">
							<el-switch v-model="scope.row.status" @change="changeSwitch($event, scope.row)" :loading="scope.row.$switch_yx" :active-value="true" :inactive-value="false"></el-switch>
						</template>
					</el-table-column>
		...
		methods: {
			...
			async getDic(){
				...
				if(firstNode){
					...
					this.listApiParams = {
						dic__code: firstNode.code
					}
					this.listApi = this.$API.badmin.dictvalue.list;
				}
			},
			//树过滤
			...
			async table_del(row, index){
				...
				var res = await this.$API.badmin.dictvalue.remove.delete(reqData.id);
				if(res.status == 204){
			...
			async batch_del(){
				this.$confirm(`确定删除选中的 ${this.selection.length} 项吗？`, '提示', {
					...
					this.$API.badmin.dictvalue.remove.batch_delete({ids: this.ids}).then(res => {
						if (res.status == 204){
			...
			// 		var res = await this.getSubmitApi(this.mode, formData);
			// 		console.log(this.mode, formData)
			...
			changeSwitch(val, row){
				...
				row.$switch_yx = true;
				//3.等待接口返回后改变值
				this.$API.badmin.dictvalue.partial_update.patch(row.id, {status: val}).then(res => {
					console.log(res)
					if (res.status == 200){
			...
			handleDicSuccess(data, mode){
				if(mode=='add'){
					...
					//判断是否移动？
					var editNodeParentId =  editNode.level==1?undefined:editNode.parent.data.id
					if(editNodeParentId != data.parent){
						var obj = editNode.data;
						this.$refs.dic.remove(data.id)
						this.$refs.dic.append(obj, data.parent)
					}

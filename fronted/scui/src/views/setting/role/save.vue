	...
	export default {
		...
		methods: {
			...
			submit(){
				this.$refs.dialogForm.validate(async (valid) => {
					if (valid) {
						...
						let _dataForm = {
							id: this.form.id,
							codename: this.form.alias,
							name: this.form.label,
							sort: this.form.sort,
							status: this.form.status,
							remark: this.form.remark
						}
						var res = await this.getSubmitApi(this.mode, _dataForm);
						...
						if (res.status == 200 || res.status == 201){
							...
							this.$alert(res.data.detail, "提示", {type: 'error'})
						}
					}
				})
			},
			...
			setData(data){
				...
				this.form.label = data.group.name
				this.form.alias = data.codename
			...
			getSubmitApi(model, data){
				if (model == 'edit'){
					return this.$API.badmin.roles.update.put(data.id, data)
				}else if (model == 'add'){
					return this.$API.badmin.roles.create.post(data)
				}
			}
		}
	}

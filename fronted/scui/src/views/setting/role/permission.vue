	...
	export default {
		...
		methods: {
			...
			submit(){
				...
				// console.log(checkedKeys, 'menus')
				// 部门选择
				var checkedKeys_dept = this.$refs.dept.getCheckedKeys().concat(this.$refs.dept.getHalfCheckedKeys())
				// console.log(checkedKeys_dept, 'dept')
				// console.log(this.data.dataType, '权限范围')
				// 控制台组件
			...
			async getMenu(){
				var res = await this.$API.badmin.menus.list.get()
				this.menu.list = res.data
				//获取接口返回的之前选中的和半选的合并，处理过滤掉有叶子节点的key
				// this.menu.checked = ["83", "84"]
				this.menu.checked = this.$parent.selection[0].menus
				this.$nextTick(() => {
			...
			async getDept(){
				...
				this.data.list = res.data.results
				this.data.checked = this.$parent.selection[0].depts
				// this.data.checked = ["154", "155", "83", "84"]
				this.data.dataType = this.$parent.selection[0].perm_range
				this.$nextTick(() => {

	...
	<el-container>
		...
		<el-main class="nopadding">
			<scTable ref="table" :apiObj="apiObj" row-key="id" @selection-change="selectionChange" stripe>
				...
				<el-table-column label="角色名称" prop="group.name" width="150"></el-table-column>
				<el-table-column label="别名" prop="codename" width="200"></el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				apiObj: this.$API.badmin.roles.list,
				selection: [],
				search: {
					keyword: null
				},
		...
		methods: {
			...
			//删除
			async table_del(row){
				var res = await this.$API.badmin.roles.remove.delete(row.id);
				if(res.status == 204){
			...
			async batch_del(){
				this.$confirm(`确定删除选中的 ${this.selection.length} 项吗？如果删除项中含有子集将会被一并删除`, '提示', {
					...
					this.$API.badmin.roles.remove.batch_delete({ids:this.ids}).then(res => {
						if (res.status == 204){
							this.$refs.table.refresh()
							loading.close();
			...
			changeSwitch(val, row){
				...
				this.$API.badmin.roles.partial_update.patch(row.id, {status:val}).then(res => {
					if (res.status == 200){
			...
			// 向角色添加权限
			permissionSuccess(data){
				const loading = this.$loading();
				this.$API.badmin.roles.partial_update.patch(data.role_id, data).then(res => {
					if (res.status == 200){
						this.$refs.permissionDialog.isSaveing = false
						this.$refs.permissionDialog.visible = false
						this.$refs.table.refresh()
						loading.close();
						this.$message.success("操作成功")
					}else{
						loading.close();
						this.$message.error("修改失败，请检查，或联系管理员！")
					}
				})
			}
		}

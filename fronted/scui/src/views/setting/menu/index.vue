...
<script>
	...
	export default {
		...
		methods: {
			...
			//增加
			async add(node, data){
				var newMenuName = "未命名" + `${Math.floor(Math.random()*50)}` + newMenuIndex++;
				var newMenuData = {
				...
				this.menuloading = true
				try{
					var res = await this.$API.badmin.menus.create.post(newMenuData)
				}catch (error) {
					console.log(error)
				}
				this.menuloading = false
			...
			async delMenu(){
				...
				var res = await this.$API.badmin.menus.remove.batch_delete(reqData)
				this.menuloading = false

			...
			<scTable ref="table" :apiObj="apiObj" row-key="id" @selection-change="selectionChange" hidePagination>
				...
				<el-table-column label="状态" prop="is_enable" width="150">
					<template #default="scope">
						<el-tag v-if="scope.row.is_enable" type="success">启用</el-tag>
						<el-tag v-if="!scope.row.is_enable" type="danger">停用</el-tag>
					</template>
				</el-table-column>
				<el-table-column label="创建时间" prop="add_date" width="180"></el-table-column>
				<el-table-column label="备注" prop="mark" min-width="300"></el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				apiObj: this.$API.badmin.dept.list,
				selection: [],
				search: {
					keyword: null
				},
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.badmin.dept.remove.delete(row.id);
				if(res.status == 204){
			...
			async batch_del(){
				this.$confirm(`确定删除选中的 ${this.selection.length} 项吗？如果删除项中含有子集将会被一并删除`, '提示', {
					...
					this.$API.badmin.dept.remove.batch_delete({ids:this.ids}).then(res => {
						if (res.status == 204){
							this.$refs.table.refresh()
							loading.close();

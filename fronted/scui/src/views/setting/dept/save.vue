	...
	<el-dialog :title="titleMap[mode]" v-model="visible" :width="500" destroy-on-close @closed="$emit('closed')">
		<el-form :model="form" :rules="rules" :disabled="mode=='show'" ref="dialogForm" label-width="100px">
			...
			<el-form-item label="是否有效" prop="is_enable">
				<el-switch v-model="form.is_enable" :active-value="true" :inactive-value="false"></el-switch>
			</el-form-item>
	...
	export default {
		...
		methods: {
			...
			getSubmitApi(model, data){
				if (model == 'edit'){
					return this.$API.badmin.dept.update.put(data.id, data)
				}else if (model == 'add'){
					return this.$API.badmin.dept.create.post(data)
				}
			}
		}
	}

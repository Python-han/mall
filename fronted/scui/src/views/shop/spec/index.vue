	...
	<el-container>
		<el-header>
			...
			<div class="right-panel">
				<div class="right-panel-search">
					<el-input v-model="search.keyword" placeholder="规格名称" clearable></el-input>
		...
		<el-main class="nopadding">
			<scTable ref="table" :apiObj="apiObj" row-key="id" @selection-change="selectionChange">
				...
				<!-- <el-table-column label="品牌logo" prop="logo" width="250">
					<template #default="scope">
						<el-image :src="scope.row.logo" style="width: 30px; height: 30px" fit="cover" :preview-src-list="[scope.row.logo]" :preview-teleported="true">
	...
	export default {
		...
		data() {
			return {
				...
				apiObj: this.$API.shop.spec.list,
				selection: [],
				search: {
					keyword: null
				},
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.shop.spec.remove.delete(row.id);
				if(res.status == 204){
			...
			async batch_del(){
				this.$confirm(`确定删除选中的 ${this.selection.length} 项吗？如果删除项中含有子集将会被一并删除`, '提示', {
					...
					this.$API.shop.spec.remove.batch_delete({ids:this.ids}).then(res => {
						if (res.status == 204){
							this.$refs.table.refresh()
							loading.close();

	...
	<el-container>
		...
		<el-main class="nopadding">
			<scTable ref="table" :apiObj="apiObj" row-key="id" @selection-change="selectionChange">
				...
				<el-table-column label="轮播图" prop="img" width="250">
					<template #default="scope">
						<el-image :src="scope.row.img" style="width: 30px; height: 30px" fit="cover" :preview-src-list="[scope.row.logo]" :preview-teleported="true">
				...
				<el-table-column label="跳转链接" prop="target" width="250"></el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				apiObj: this.$API.shop.banner.list,
				selection: [],
				search: {
					keyword: null
				},
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.shop.banner.remove.delete(row.id);
				if(res.status == 204){
			...
			async batch_del(){
				this.$confirm(`确定删除选中的 ${this.selection.length} 项吗？如果删除项中含有子集将会被一并删除`, '提示', {
					...
					this.$API.shop.banner.remove.batch_delete({ids:this.ids}).then(res => {
						if (res.status == 204){
							this.$refs.table.refresh()
							loading.close();

<template>
    <el-container>
        ...
        <el-main class="nopadding">
            <scTable ref="table" :data="tableData" row-key="id" @selection-change="selectionChange" stripe>
				<el-table-column type="selection" width="50"></el-table-column>
                <el-table-column label="分类名称" prop="name" width="250"></el-table-column>
                <el-table-column label="别名" prop="slug" width="150"></el-table-column>
                <el-table-column label="字段类型" prop="field_type" width="150"></el-table-column>
                <el-table-column label="值" prop="value" width="150"></el-table-column>
                ...
                <el-table-column label="状态" prop="is_enable" width="150">
    ...
    export default {
        ...
        data() {
			return {
				...
				apiObj: this.$API.badmin.confcategory.list,
				selection: [],
				search: {
					keyword: null
				},
        ...
        mounted(){
			this.$API.badmin.confoptions.list.get(
        ...
        methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.badmin.confcategory.remove.delete(row.id);
				if(res.status == 204){
			...
			async batch_del(){
				this.$confirm(`确定删除选中的 ${this.selection.length} 项吗？如果删除项中含有子集将会被一并删除`, '提示', {
					...
					this.$API.badmin.confcategory.remove.batch_delete({ids:this.ids}).then(res => {
						if (res.status == 204){
							this.$refs.table.refresh()
							loading.close();

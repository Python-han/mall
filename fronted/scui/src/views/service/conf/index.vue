		...
		<el-main class="nopadding">
			<scTable ref="table" :apiObj="apiObj" row-key="id" @selection-change="selectionChange" stripe>
				...
				<el-table-column label="操作" fixed="right" align="right" width="300">
					<template #default="scope">
						<el-button-group>
							<el-button text type="primary" size="small" @click="confops(scope.row, scope.$index)">配置列表</el-button>
		...
		methods: {
			...
			confops(row){
				...
				this.$router.beforeEach((to) => {
					to.meta.title = row.name
				})
				this.$router.replace({name:'confops', params: {id: row.id}})
			}
		}
	}
</script>
